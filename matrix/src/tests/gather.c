...
START_TEST(s21_inverse_matrix_01) {
  ...
  if (s21_inverse_matrix(&A, &Z) != OK) {
    s21_remove_matrix(&A);
    ck_abort_msg("Failed to compute inverse matrix");
  }
  s21_remove_matrix(&A);

  if (s21_create_matrix(4, 4, &X) != OK) {
    s21_remove_matrix(&Z);
...
Suite *suite_matrix_gather(void) {
  ...
  tcase_add_test(tc, s21_mult_number_07);
