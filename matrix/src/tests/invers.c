...
START_TEST(inverse_3) {
  ...
  ck_assert_double_eq(B.matrix[1][0], 1.5);
  ck_assert_double_eq(B.matrix[1][1], -0.5);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  ...
  ck_assert_double_eq(B.matrix[0][0], 1 / 200.0);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  ...
  ck_assert_int_eq(s21_inverse_matrix(&A, &B), INCORRECT);
}
END_TEST
